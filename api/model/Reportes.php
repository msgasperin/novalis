	...
	class Reportes extends Conexion {
		...
		public function cuentas_por_cobrar(string $fechaIni, string $fechaFinal, int $id_sucursal) {
			...
			try {
				...
				$sqlStr = "SELECT 
						...
						WHERE DATE(fecha_cap) BETWEEN ? AND ? 
		...
		public function estudios_rentabilidad(string $fechaIni, string $fechaFinal, int $id_sucursal) {
			...
			try {
				...
				$whereSucursal = "";
				if ($id_sucursal > 0) {
					// Si necesitas filtrar por sucursal, nos unimos a ordenes_trabajo sólo en ese caso
					// o si orden_detalles ya tiene sucursal_id, se filtra directamente.
					$whereSucursal = " AND ot.sucursal_id = ? ";
					$params[] = $id_sucursal;
				}

				// Si no filtramos por sucursal, la consulta va directa a orden_detalles
				$fromClause = "FROM orden_detalles od";
				if ($id_sucursal > 0) {
					$fromClause .= " INNER JOIN ordenes_trabajo ot ON od.orden_id = ot.id";
				}

				$sqlStr = "SELECT 
							od.estudio_id,
							od.nombre_estudio_historico AS item_nombre,
							COUNT(od.id) AS cantidad_vendida,
							SUM(od.precio_aplicado) AS ingreso_total,
							AVG(od.precio_aplicado) AS precio_promedio,
							SUM(COALESCE(od.costo_aplicado, 0)) AS costo_total,
							SUM(COALESCE(od.utilidad, (od.precio_aplicado - COALESCE(od.costo_aplicado, 0)))) AS utilidad_bruta,
							CASE 
								WHEN SUM(od.precio_aplicado) > 0 
								THEN ROUND((SUM(COALESCE(od.utilidad, (od.precio_aplicado - COALESCE(od.costo_aplicado, 0)))) / SUM(od.precio_aplicado)) * 100, 2)
								ELSE 0 
							END AS porcentaje_margen
						$fromClause
						WHERE DATE(od.fecha_cap) BETWEEN ? AND ? 
							AND od.estatus_orden != 'CANCELADO'
							$whereSucursal
						GROUP BY od.estudio_id, od.nombre_estudio_historico
		...
		public function rendimiento_clientes(string $fechaIni, string $fechaFinal, int $id_sucursal) {
			...
			try {
				...
				error_log("Error en rendimiento_convenios: " . $error->getMessage());
			}
		...
		public function analisis_pacientes(string $fechaIni, string $fechaFinal, int $id_sucursal) {
			...
			try {
				...
				// -----------------------------------------------------------------
				// 1. CONSULTA DEMOGRÁFICA: Rangos de Edad (usando paciente_edad INT) y Sexo
				// -----------------------------------------------------------------
				$sqlDemografia = "SELECT 
											CASE 
												WHEN ot.paciente_edad BETWEEN 0 AND 12 THEN '0 - 12 (Pediátrico)'
												WHEN ot.paciente_edad BETWEEN 13 AND 17 THEN '13 - 17 (Adolescentes)'
												WHEN ot.paciente_edad BETWEEN 18 AND 35 THEN '18 - 35 (Adulto Joven)'
												WHEN ot.paciente_edad BETWEEN 36 AND 59 THEN '36 - 59 (Adulto)'
												WHEN ot.paciente_edad >= 60 THEN '60+ (Adulto Mayor)'
												ELSE 'No Especificado'
											END AS rango_edad,
											UPPER(TRIM(ot.paciente_sexo_historico)) AS sexo,
											COUNT(ot.id) AS total_atenciones,
											COUNT(DISTINCT ot.paciente_id) AS pacientes_unicos,
											SUM(ot.total_neto) AS total_facturado
										FROM ordenes_trabajo ot
										WHERE DATE(ot.fecha_cap) BETWEEN ? AND ?
										AND ot.estatus != 'CANCELADO'
										$whereSucursal
										GROUP BY rango_edad, UPPER(TRIM(ot.paciente_sexo_historico))
										ORDER BY MIN(ot.paciente_edad) ASC";
		...
		public function auditoria_cancelaciones(string $fechaIni, string $fechaFinal, int $id_sucursal) {
			...
			try {
				...
				$sqlKPIs = "SELECT 
								...
								FROM ordenes_trabajo 
								WHERE DATE(fecha_cap) BETWEEN ? AND ? $whereSucursal";
				...
				// -----------------------------------------------------------------
				// 2. DETALLE DE ÓRDENES CANCELADAS (Con Motivo y Usuario Cancela)
				// -----------------------------------------------------------------
